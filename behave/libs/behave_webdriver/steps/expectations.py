...
if 'transform-parse' not in matcher_mapping:
    ...
    behave.use_step_matcher('transform-re')


@behave.given('the element "([^"]*)?" is( not)* visible')
@behave.then('I expect that element "([^"]*)?" becomes( not)* visible')
@behave.then('I expect that element "([^"]*)?" is( not)* visible')
...
@behave.given('the title is( not)* "([^"]*)?"')
@behave.then('I expect that the title is( not)* "([^"]*)?"')
...
@behave.then('I expect that element "([^"]*)?" is( not)* within the viewport')
...
@behave.given('the element "([^"]*)?" is( not)* enabled')
@behave.then('I expect that element "([^"]*)?" is( not)* enabled')
...
@behave.given('the element "([^"]*)?" is( not)* selected')
@behave.then('I expect that element "([^"]*)?" is( not)* selected')
...
@behave.given('the checkbox "([^"]*)?" is( not)* checked')
@behave.then('I expect that checkbox "([^"]*)?" is( not)* checked')
...
@behave.given('there is (an|no) element "([^"]*)?" on the page')
...
@behave.then('I expect that element "([^"]*)?" does( not)* exist')
...
@behave.given('the element "([^"]*)?" contains( not)* the same text as element "([^"]*)?"')
@behave.then('I expect that element "([^"]*)?"( not)* contains the same text as element "([^"]*)?"')
...
@behave.given('the element "([^"]*)?"( not)* matches the text "([^"]*)?"')
@behave.then('I expect that element "([^"]*)?"( not)* matches the text "([^"]*)?"')
...
@behave.given('the element "([^"]*)?"( not)* contains the text "([^"]*)?"')
@behave.then('I expect that element "([^"]*)?"( not)* contains the text "([^"]*)?"')
...
@behave.given('the element "([^"]*)?"( not)* contains any text')
@behave.then('I expect that element "([^"]*)?"( not)* contains any text')
...
@behave.given('the element "([^"]*)?" is( not)* empty')
@behave.then('I expect that element "([^"]*)?" is( not)* empty')
...
@behave.given('the page url is( not)* "([^"]*)?"')
@behave.then('I expect that the url is( not)* "([^"]*)?"')
...
@behave.then('I expect the url to( not)* contain "([^"]*)?"')
...
@behave.given('the( css)* attribute "([^"]*)?" from element "([^"]*)?" is( not)* "([^"]*)?"')
@behave.then('I expect that the( css)* attribute "([^"]*)?" from element "([^"]*)?" is( not)* "([^"]*)?"')
...
@behave.given('the cookie "([^"]*)?" contains( not)* the value "([^"]*)?"')
@behave.then('I expect that cookie "([^"]*)?"( not)* contains "([^"]*)?"')
...
@behave.given('the cookie "([^"]*)?" does( not)* exist')
...
@behave.then('I expect that cookie "([^"]*)?"( not)* exists')
...
@behave.given('the element "([^"]*)?" is( not)* ([\d]+)px (broad|tall)')
@behave.then('I expect that element "([^"]*)?" is( not)* ([\d]+)px (broad|tall)')
...
@behave.given('the element "([^"]*)?" is( not)* positioned at ([\d]+)px on the (x|y) axis')
@behave.then('I expect that element "([^"]*)?" is( not)* positioned at ([\d]+)px on the (x|y) axis')
...
@behave.given('a (alertbox|confirmbox|prompt) is( not)* opened')
@behave.then('I expect that a (alertbox|confirmbox|prompt) is( not)* opened')
...
@behave.then('I expect that the path is( not)* "([^"]*)?"')
...
@behave.then('I expect that element "([^"]*)?" (has|does not have) the class "([^"]*)?"')
...
@behave.then('I expect a new (window|tab) has( not)* been opened')
...
@behave.then('I expect the url "([^"]*)?" is opened in a new (tab|window)')
...
@behave.then('I expect that element "([^"]*)?" is( not)* focused')
...
@behave.then('I expect that a (alertbox|confirmbox|prompt)( not)* contains the text "([^"]*)?"')
...
@behave.then('I wait on element "([^"]*)?"(?: for (\d+)ms)*(?: to( not)* (be checked|be enabled|be selected|be visible|contain a text|contain a value|exist))*')
...
@behave.then("I expect the screen is ([\d]+) by ([\d]+) pixels")

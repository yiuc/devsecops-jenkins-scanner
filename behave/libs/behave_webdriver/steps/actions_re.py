...
@behave.when('I close the last opened (tab|window)')
...
@behave.when('I focus the last opened (tab|window)')
def focus_last_tab(context, _):
    context.behave_driver.switch_to_window(context.behave_driver.last_opened_handle)


@behave.when('I select the option with the (text|value|name) "([^"]*)?" for element "([^"]*)?"')
...
@behave.step('I open the url "([^"]*)?"')
def open_url(context, url):
    context.behave_driver.open_url(url)


@behave.step('I open the site "([^"]*)?"')
